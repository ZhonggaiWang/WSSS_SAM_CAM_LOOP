...
from utils.camutils import single_instance_crop_cam,single_bg_fg_crop_denoise_v3,cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads
...
parser.add_argument("--w_seg", default=0.17, type=float, help="w_seg")
...
def validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            _cams, _cams_aux = multi_scale_cam2(model, inputs, (1.0, 0.5, 0.75, 1.5))
...
def train(args=None):
    ...
    train_dataset = voc.VOC12AblDataset(
...
#pretrained_load——————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    # trained_state_dict = torch.load('/home/zhonggai/python-work-space/DEFormer/DEFormer/scripts/work_dir_voc_wseg/2024-07-26-21-45-19-184299/checkpoints/default_model_iter_2000.pth', map_location="cpu")
    ...
    b1_contrast_prototype = ContrastLoss_prototype_instance()
    b2_contrast_prototype = ContrastLoss_prototype_instance()
...
#add param ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            img_name, inputs, cls_label, img_box, crops,image_origin,gt_label = next(train_loader_iter)
...
#get_local_pic_to_encode------------------------------------------------------------------------------
        ...
        b1_roi_mask[b1_roi_mask == -1] = -2
        b1_local_crops, b1_flags, b1_box= single_instance_crop_cam(images=inputs_denorm, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= b1_roi_mask, boundary_box=img_box)
        b2_roi_mask[b2_roi_mask == -1] = -2
        b2_local_crops, b2_flags, b2_box= single_instance_crop_cam(images=inputs_denorm, cls_label = cls_label,roi_mask_28x=b2_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= b2_roi_mask, boundary_box=img_box)
...
# branch2 : adaptive-label-bce + cls loss part-------------------------------------------------------------------------------------------------------------------
        ...
        if n_iter <= 4000:
            ...
            import math
            shedule = math.cos((n_iter-4000)/4000 * math.pi/2) * 0.1
...
#train------------------------------------------------------------------------------------------------------------------------
        if n_iter <= 2000:
            ...
            loss = (0.4 * b2_spacial_bce_loss + 0.6 * b2_cls_loss) + 1.0 * b1_cls_loss+ 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * network_sim_loss + 0.05 * contrast_loss
