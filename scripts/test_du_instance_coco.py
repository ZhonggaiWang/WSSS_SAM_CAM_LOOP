...
from model.losses import get_masked_ptc_loss, get_seg_loss, ContrastLoss_prototype_instance, DenseEnergyLoss, get_energy_loss,CPCLoss,get_spacial_bce_loss,ContrastLoss_moco,ContrastLoss_prototype,get_bg_fg_contrastive_clamp_loss,get_bg_fg_contrastive_loss
...
from utils.camutils import single_instance_crop_v2,single_instance_crop_denoise,cam_to_label,cam_to_roi_mask_uncertain_assign, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads,select_local_patch
...
parser.add_argument("--img_folder", default='../MSCOCO', type=str, help="dataset folder")
parser.add_argument("--label_folder", default='../MSCOCO', type=str, help="dataset folder")
...
parser.add_argument("--w_seg", default=0.15, type=float, help="w_seg")
...
parser.add_argument("--t_b1_mix_cam", default=0.5, type=float, help="t_b1_mix_cam")
...
def train(args=None):
    ...
    dist.init_process_group(backend=args.backend, timeout=datetime.timedelta(seconds=60000))
...
#pretrained_load——————————————————————————————————————————————————————————————————————————————————————————————————
    b1_contrast_prototype = ContrastLoss_prototype_instance(temp=0.5,num_cls=80)
    b2_contrast_prototype = ContrastLoss_prototype_instance(temp=0.5,num_cls=80)
...
#get pseudo label --------------------------------------------------------------------------------------------------------------------------------
        _roi_mix_cam = 0.5 * b1_cams + 0.5 * b2_cams
        
        
        roi_mix_valid_cam, _ = cam_to_label(_roi_mix_cam.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        refined_roi_mix_pesudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=roi_mix_valid_cam, cls_labels=cls_label,  high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index, img_box=img_box, )
        #训练加速
        # _4x_cam = F.interpolate(_roi_mix_cam.detach(), size=(int(_roi_mix_cam.shape[2]/4),int(_roi_mix_cam.shape[3]/4)), mode="bilinear", align_corners=False)
        # _4x_roi_mask = cam_to_roi_mask2(_4x_cam.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)
        
        
        # _roi_mask_denoise = cam_to_roi_mask_uncertain_assign(_roi_mix_cam.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)

        
        
        b1_roi_mask = cam_to_roi_mask2(_roi_mix_cam.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)
        b2_roi_mask = cam_to_roi_mask2(_roi_mix_cam.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)
        

        
        # _,b1_28x_pseudo_label = cam_to_label_resized(_roi_mix_cam.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index,printornot=False,clip = False)
        # _,b2_28x_pseudo_label = cam_to_label_resized(_roi_mix_cam.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index,printornot=False,clip = False)
...
#get_local_pic_to_encode------------------------------------------------------------------------------
        ...
        n_crops = args.local_crop_num
        b1_local_crops, b1_flags, b1_box= single_instance_crop_v2(images=inputs, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label,boundary_box=img_box)
        b2_local_crops, b2_flags, b2_box= single_instance_crop_v2(images=inputs, cls_label = cls_label,roi_mask_28x=b2_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label,boundary_box=img_box)
        
        # b1_local_crops, b1_flags, b1_box= single_instance_crop_denoise(images=inputs, cls_label = cls_label,roi_mask_uncertain_assign=_roi_mask_denoise, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label,boundary_box=img_box)
        # b2_local_crops, b2_flags, b2_box= single_instance_crop_denoise(images=inputs, cls_label = cls_label,roi_mask_uncertain_assign=_roi_mask_denoise, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label,boundary_box=img_box)
        
        
        
        local_pic = (b1_local_crops , b2_local_crops)
        # print(len(b1_local_crops))
# model forward-------------------------------------------------------------------------------------------------------------------------------

        cls, segs, fmap, cls_aux, cam_12th, cls_token, contrast_feature = model(inputs, _28x_pseudo_label,crops='#', n_iter=n_iter,select_k = 1,return_cls_token = True,local_pic = local_pic)    
        ...
        (b1_contrast_feature,b2_contrast_feature) = contrast_feature
        # b1_contrast_feature = contrast_feature

#contrast learning -----------------------------------------------------------------------

        # if n_iter >= 8000:
        b1_prototyped_loss = b1_contrast_prototype(b1_cls_token,b1_contrast_feature,cls_label,b1_flags)
        b2_prototyped_loss = b2_contrast_prototype(b2_cls_token,b2_contrast_feature,cls_label,b2_flags)
        contrast_loss = (b1_prototyped_loss + b2_prototyped_loss)
            # print("prototype",contrast_loss) 
        # else:
...
# branch2 : adaptive-label-bce + cls loss part-------------------------------------------------------------------------------------------------------------------
        ...
        # else:
        aux_pesedo_show = b2_refined_aux_pesudo_label = cam_to_roi_mask2(_roi_mix_cam.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)
        # shedule = 0.1 * (1 - (n_iter-4000) / 4000)
        import math
        shedule = math.cos(n_iter/20000 * math.pi/2) * 0.1
        per_pic_thre = get_per_pic_thre(b2_refined_aux_pesudo_label, gd_label=cls_label, uncertain_region_thre = shedule)
...
#b1 b2 generate pesudo-label and seg------------------------------------------------------------------------------------------------------------------------------------------
        ...
        b1_aux_mix_cams = args.t_b1_mix_cam * b1_cams_aux.detach() + (1-args.t_b1_mix_cam) * b2_cams_aux.detach()
        ...
        b1_valid_aux_cam, _ = cam_to_label(b1_aux_mix_cams.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        # if n_iter >= 8000: 
        ...
        # else:
        #     b1_refined_pseudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=b1_valid_aux_cam, cls_labels=cls_label,  high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index, img_box=img_box, )
        ...
        b2_aux_mix_cams = args.t_b2_mix_cam * b2_cams_aux.detach() + (1-args.t_b2_mix_cam) * b1_cams_aux.detach()
        ...
        b2_valid_aux_cam, _ = cam_to_label(b2_aux_mix_cams.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        # if n_iter >= 8000: 
        ...
        # else:
        #     b2_refined_pseudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=b2_valid_aux_cam, cls_labels=cls_label,  high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index, img_box=img_box, )
...
#train------------------------------------------------------------------------------------------------------------------------
        # if n_iter <= 6000:
        #     loss = 1.0 * (b1_cls_loss + b2_cls_loss) + 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + 0 * ptc_loss  + 0.0 * seg_loss
        # elif n_iter <= 8000:
        #     loss = 1.0 *  (b1_cls_loss + b2_cls_loss) + 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + 0 * ptc_loss + args.w_seg * seg_loss 
        # elif n_iter <= 16000:
        ...
        # else: 
        loss = (0.3 * b2_spacial_bce_loss + 0.7 * b2_cls_loss) + 1.0 * b1_cls_loss+ 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * network_sim_loss + 0.05 * contrast_loss
