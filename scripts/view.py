...
parser.add_argument("--list_folder", default='./datasets/voc_split', type=str, help="train/val/test list file")
...
parser.add_argument("--train_set", default="class_0", type=str, help="training split")
...
def validate_sam_mask(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            mask_cls_list, _, fmap, mask_cls_aux_list, cam_12th, cls_token,mask_cam_list,aux_mask_cam_list = model(inputs, "#",crops='#',img_name = None,sam_mask=sam_mask)   
            sam_mask_label = get_sam_mask_label(mask_cam_list=mask_cam_list, cls_label=cls_label,sam_mask=sam_mask, bg_thre=0.72)
...
def train(args=None):
    ...
    trained_state_dict = torch.load('/home/zhonggai/python-work-space/ToCo-B_WSSS_work/CVPR_wsss_baseline/work_dir_voc_wseg/mask_cls/checkpoints/model_iter_20000.pth', map_location="cpu")
...
#add param ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    for n_iter in range(args.max_iters):
        ...
        import matplotlib.pyplot as plt
        cmap = plt.get_cmap()  # 100 个类别，使用tab20颜色映射
        # 可视化 mask
        plt.figure(figsize=(8, 8))
        plt.imshow(sam_mask[0].cpu(), cmap=cmap)
        plt.colorbar()
        plt.savefig("sam_mask")
        plt.close()
        numpy_image = inputs_denorm[0].permute(1, 2, 0).cpu().numpy()
        pil_image = Image.fromarray((numpy_image * 255).astype('uint8'))
        pil_image.save('inputs_image.png')
        
        label_path = '/home/zhonggai/python-work-space/ToCo-B_WSSS_work/CVPR_wsss_baseline/VOC2012/SegmentationClass'
        gt_label = os.path.join(label_path, img_name[0])
        gt_label = Image.open(gt_label+'.png')
        gt_label.save("gt label.png")
...
# cls loss & aux cls loss =================================================================================================================
        ...
        cls_sig = F.sigmoid(cls)
        cls_pred[cls_sig>0.6] = 1
...
#per-pixel refine CAM with GT/SAM/SPL  =====================================================================================================
...
# seg_loss & reg_loss==========================================================================================================================
        ...
        if n_iter>= args.warmup_iters:
            refined_pseudo_label = sam_mask_label = get_sam_mask_label(mask_cam_list,cls_label,sam_mask,0.7,mask_cls_list=mask_cls_list) #only cls label
        ...
        plt.imshow(refined_pseudo_label[0].cpu().numpy(), cmap='jet', vmin=0, vmax=20)
        plt.colorbar()
        plt.savefig("sam_mask_label")
        plt.close()    
        ...
        seg_pred = torch.argmax(segs, dim=1).cpu().numpy().astype(np.int16)
        import imageio
...
# ptc loss=======================================================================================================================================
        ...
        # warmup
        if n_iter <= 2000:
            loss = 0.0 * cls_loss + 0.0 * cls_loss_aux + 0 * ptc_loss + 0.0 * seg_loss 
        else:
            loss = 0.0 * cls_loss + 0.0 * cls_loss_aux + 0 * ptc_loss + args.w_seg * seg_loss 
