...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __transforms(self, image):
        ...
        return image, local_image, img_box,image_origin
    ...
    def __getitem__(self, idx):
        ...
        image, local_image, img_box,image_origin = self.__transforms(image=image)
        ...
        if self.aug:
            ...
            return img_name, image, cls_label, img_box, crops,image_origin
        else:
            return img_name, image, cls_label



#ablation study PCC with gt labels
class VOC12AblDataset(VOC12Dataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
    ...
    def __transforms(self, image,label):
        ...
        if self.aug:
            ...
            if self.crop_size:
                image, label,img_box = transforms.random_crop(image,label, crop_size=self.crop_size, mean_rgb=[0,0,0], ignore_index=self.ignore_index)
            image_origin = image
            local_image = self.local_view(Image.fromarray(image))
            image = self.global_view1(Image.fromarray(image))
        ...
        return image, local_image, img_box,image_origin,label
    ...
    def __getitem__(self, idx):
        ...
        image, local_image, img_box,image_origin,label = self.__transforms(image=image,label=label)
        ...
        if self.aug:
            ...
            return img_name, image, cls_label, img_box, crops,image_origin,label
        else:
            return img_name, image, cls_label
...
class VOC12SamClsDataset(VOC12Dataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
        ...
        self.img_fliplr = img_fliplr
        self.num_classes = num_classes
        self.sam_mask_path = "/home/zhonggai/python-work-space/SAM-B_WSSS_work/S2C-main/se/default"
        # self.color_jittor = transforms.PhotoMetricDistortion()
    ...
    def __transforms(self, image, image_name):
        
        
        
        sam_mask_path = self.sam_mask_path
        sam_current_mask_path = sam_mask_path +'/' + image_name + '.npy'
        sam_mask = np.load(sam_current_mask_path)
        ...
        if self.aug:
            ...
            if self.rescale_range:
                image , sam_mask= transforms.random_scaling(image,sam_mask, scale_range=self.rescale_range)
            if self.img_fliplr:
                image, sam_mask  = transforms.random_flip_mask(image,None,sam_mask)
            if self.crop_size:
                image, sam_mask, img_box = transforms.random_crop(image, sam_mask, crop_size=self.crop_size, mean_rgb=[0,0,0], ignore_index=self.ignore_index)
            image_origin = image
            local_image = self.local_view(Image.fromarray(image))
            image = self.global_view1(Image.fromarray(image))
        ...
        return image, local_image, img_box,image_origin, sam_mask
    ...
    def __getitem__(self, idx):
        ...
        image, local_image, img_box,image_origin, sam_mask = self.__transforms(image=image, image_name = img_name)
        ...
        if self.aug:
            ...
            return img_name, image, cls_label, img_box, crops,image_origin, sam_mask
        else:
            return img_name, image, cls_label
...
class VOC12SegDataset(VOC12Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sam_mask_path = self.sam_mask_path
        sam_current_mask_path = sam_mask_path +'/' + img_name + '.npy'
        sam_mask = np.load(sam_current_mask_path)
        ...
        return img_name, image, label, cls_label, sam_mask