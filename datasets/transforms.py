...
def random_scaling(image, label=None, scale_range=None):
    ...
    return _img_mask_rescaling(image, label, scale=ratio)


def _img_mask_rescaling(image, mask=None, scale=None):
    ...
    import cv2
    new_mask = cv2.resize(mask, new_scale, interpolation=cv2.INTER_NEAREST)
...
def random_flip_mask(image, label=None, mask = None):
    p = random.random()
